...
- 定义：统计周期内项目新增的 Stars 数量，即本周期累计 Star 总数与上周期累计 Star 总数的差值。
- 权重：50%
- 阈值：100 个

Stars 数量是衡量项目受欢迎程度的直观指标。该指标统计周期内项目新增的 Star 数量，反映了项目在开发者社区中的关注度和吸引力。持续增长的 Stars 数量表明项目具有良好的市场影响力和用户基础。

### 项目 Forks 新增

- 定义：统计周期内项目新增的 Forks 数量，即本周期累计 Fork 总数与上周期累计 Fork 总数的差值。
- 权重：50%
- 阈值：100 个

Forks 数量反映了开发者对项目的参与意愿。该指标统计周期内项目新增的 Fork 数量，表明开发者对项目感兴趣，希望基于项目进行二次开发或贡献代码。这是评估项目活跃度和开发者参与度的重要指标。
...
- 定义：统计周期内新增的代码提交（Commit）总次数，按提交哈希去重统计。
- 权重：20%
- 阈值：12850 个

代码提交次数是衡量项目开发活跃度的核心指标。该指标统计周期内社区贡献者提交的 Commit 总数，高频次的代码提交表明项目处于积极开发状态，社区贡献者持续投入。该指标反映了项目的整体工作量和发展速度。

### 新增代码行数

- 定义：统计周期内代码行变动总量，包括新增行数与修改行数之和。
- 权重：20%
- 阈值：300000 行

新增代码行数反映了项目的代码贡献规模。该指标统计周期内代码的新增与修改行数总量，虽然代码行数不能直接衡量代码质量，但它能够反映社区的开发投入程度。持续增长的代码行数表明项目在不断演进和完善。

### PR 评论数量

- 定义：统计周期内 PR 上产生的评论次数（排除机器人评论）。
- 权重：20%
- 阈值：10 个

PR 评论数量反映了社区的代码审查活跃度。该指标统计周期内 PR 上的评论总数，积极的 PR 评论表明社区重视代码质量和协作开发。该指标能够反映社区的协作氛围和技术交流深度。

### Issue 建立数量

- 定义：统计周期内新建的 Issue 总数（不含作为 PR 的记录）。
- 权重：20%
- 阈值：10 个

Issue 建立数量反映了社区的问题反馈活跃度。该指标统计周期内社区新建的 Issue 总数，该指标能够反映用户对项目的关注程度和参与意愿。

### Issue 评论数量

- 定义：统计周期内 Issue 下的评论总数（排除机器人评论）。
- 权重：20%
- 阈值：10 个

Issue 评论数量反映了社区的问题讨论活跃度。该指标统计周期内 Issue 下的评论总数，积极的 Issue 评论表明社区具有良好的沟通氛围和问题解决能力。该指标能够反映社区的协作效率和用户支持质量。
...
- 定义：统计周期内有代码提交、PR 合并或 PR 评论等代码相关贡献行为的去重用户数。
- 权重：33%
- 阈值：1000 个

代码贡献者数量反映了社区的技术贡献能力。该指标统计周期内参与代码相关贡献的去重用户数，贡献行为包括以下类型：

- 代码提交者（Committer）：将代码提交（Commit）合入仓库的提交者。

- PR 创建者：通过创建 PR 提交代码变更。

- PR 合并者：将 PR 合并到目标分支的贡献者。

- PR 评论者： 对PR进行评论的贡献者。

代码贡献者是推动项目技术发展的核心力量。该指标能够反映项目的技术吸引力和开发活跃度。
...
4 个指标采用差异化权重分配：社区贡献者数量与代码贡献者数量各为 33%，非代码贡献者数量与社区关注者数量各为 17%。
